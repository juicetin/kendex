...
pub(crate) fn content_hash(input: &AuditInput) -> String {
    ...
    match &input.content {
        ...
        Content::Hook {
            ...
            // Each field digested before the join, so a field carrying the
            // join character cannot move a boundary between fields — the
            // review hash's posture. The script contributes its text, not
            // its path: the same bytes at a moved path are the same
            // content, the way a relocated skill tree is.
            let script = script.as_ref().map(|(_, text)| text.as_str());
            let fields = [
                event.as_str(),
            ...
            .map(|field| crate::hash::hash_bytes(field.as_bytes()));
            material.push_str(&fields.join("|"));
        }

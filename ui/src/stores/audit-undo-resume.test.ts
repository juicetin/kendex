// Taking a dismissal back is one write per record. A dismissal is
...
// Commands that write in stages: an adoption moves files and records them
// before it plans what to do next, a fork captures its copy before it
// renders. Either can answer with an error and the file already changed,
// and nothing in the answer says which — so the editor is told either way.
describe("a command that wrote before it failed", () => {
  it("tells the editor even though it answered with an error", async () => {
    vi.mocked(commands.adoptItem).mockResolvedValue({
      status: "error",
      error: "the follow-up plan would not settle",
    });

    await useAuditStore.getState().adopt(scope, "skill", "gh", "claude");

    // The marks come off the file as it is now, not as it was before the
    // adoption's first stage landed.
  ...
  it("tells the editor when the command never answered at all", async () => {
    vi.mocked(commands.adoptItem).mockRejectedValue(
    ...
    // And the reader is told, rather than the rejection ending the action
    // with nothing said.

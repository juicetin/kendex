...
- **Consumer propagation is explicit.** `vstack propagate` updates recorded remote source caches, compares lock hashes, normalizes legacy missing hashes via refresh, verifies installs, and optionally stages only vstack-managed project artifacts for a consumer PR.

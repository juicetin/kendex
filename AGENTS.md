...
├── commands/            add, remove, list, check, update, update_pi, verify, refresh, propagate, init
...
- **Consumer propagation is explicit.** `vstack propagate` updates recorded remote source caches, compares lock hashes, normalizes legacy missing hashes via refresh, verifies installs, and optionally stages only vstack-managed project artifacts for a consumer PR. Recorded remotes must use GitHub shorthand, HTTPS, SSH, `git+ssh`, or `git@`; plaintext HTTP is refused.
...
- **Scope flag is uniform.** `list`, `check`, `refresh`, `remove`, `propagate` accept `--scope project|global|all`; `-g` = `--scope global`. Default: `all` for read-only, `project` for `remove` and `propagate`. Bare `vstack refresh` reinstalls items at every scope they're locked at; `vstack propagate --check` exits non-zero when drift or legacy hashes need refresh.
